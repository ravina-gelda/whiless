...
def print_command(node):
    if node.op in ("INT", "ARR", "VAR", "SKIP"):
        ...
        return ' '.join(
            ['if', str(print_command(node.cond)), 'then', '{', str(print_command(node.iftrue)), '}', 'else', '{',
             str(print_command(node.iffalse)), '}'])
    else:
...
def evaluate_print(ast, state, print_var, print_state, print_step, init_step):
    ...
    if node.op in ("INT", "ARR", "BOOL"):
        ...
        if var in state:
            ...
            state.update(
                create_dict(var, evaluate_print(node.right, state, print_var, print_state, print_step, init_step)))
        temp_var = set(print_var)
        temp_state = copy.deepcopy(state)
        temp_state = dict((var, temp_state[var]) for var in temp_var)
        print_state.append(temp_state)
        ...
        # print("and", state)
        # print_state.append(copy.deepcopy(state))
        return (evaluate_print(node.left, state, print_var, print_state, print_step, init_step) and evaluate_print(
            node.right, state, print_var, print_state, print_step, init_step))
    elif node.op == "OR":
        # print("or",state)
        # print_state.append(copy.deepcopy(state))
        return (evaluate_print(node.left, state, print_var, print_state, print_step, init_step) or evaluate_print(
            node.right, state, print_var, print_state, print_step, init_step))
    elif node.op == "WHILE":
        cond = node.cond
        wtrue = node.wtrue
        ...
        while evaluate_print(cond, state, print_var, print_state, print_step, init_step):
            ...
            temp_state = dict((var, temp_state[var]) for var in temp_var)
            print_state.append(temp_state)
            init_step = init_step.replace(print_command(node),
                                          str(print_command(node.wtrue) + '; ' + print_command(node)))
